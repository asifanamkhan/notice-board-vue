        ...
        <div class="board">
            <div class="card" v-for="(column, index) in columns" :key="index">
                <div class="card__content">
                    ...
                    <ul class="card__list" v-if="column.cards.length > 0">
                        <li class="card__item" v-for="(card, i) in column.cards" :key="i">
                            ...
                            <div class="card__item-action">
                                <span class="card__item-action-up" v-if="i > 0" @click="moveUp(i, column)"><i class="fas fa-arrow-up"></i></span>
                                <span class="card__item-action-down" v-if="i < (column.cards.length - 1)" @click="moveDown(i, column)"><i class="fas fa-arrow-down"></i></span>
...
<script>
  export default {
    ...
    methods: {
        ...
        moveUp: function(i, column) {
            let currentCrad = column.cards[i]
            let newCard = column.cards[i - 1]

            let data = {
                column_id: column.id,
                from: currentCrad.order,
                to: newCard.order
            }
